...
- Each traveller, possible booker, and provider supplied in the dedicated sender field receives a separate sender/from-address search in every month window using exactly the text entered. Names such as `Expedia` are never expanded or replaced with inferred email addresses, website domains, or regional domains. Search hints do not create additional sender constraints.
- Each month receives ten separate sender-unconstrained logical-AND searches: booking AND flight, booking AND car, booking AND ride, booking AND train, booking AND transit, booking AND ticket, booking AND reservation, booking AND experience, booking AND confirmation, and booking AND show. The two words are independent terms, not a quoted literal phrase.
...
- Results are reviewed by relevance rather than recency. When a connector cannot rank by relevance, its exposed bounded-query results are ranked for trip relevance before review. Every returned result is analyzed, while supplied search hints increase confidence without becoming exclusion filters.
