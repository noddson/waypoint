...
export function AgendaDayWeather({plans,state,today,temperatureUnit,language}:{plans:WeatherDayPlan[];state:WeatherForecastState;today:string;temperatureUnit:WeatherTemperatureUnit;language:LanguageCode}) {
  ...
  const hasWeather=plans.some(plan=>!!weatherForPlan(plan,state)),places=formatPlaceList(plans.map(plan=>plan.target.label),language),historical=plans.every(plan=>isHistoricalWeatherDate(plan.date,today))
  return <aside className="agenda-day-weather" aria-label={uiMessage('Weather for {places} on {date}',language,{places,date:shortDate(plans[0].date,language)})}>
   <div className="agenda-day-weather-reports">{plans.map(plan=>{const weather=weatherForPlan(plan,state),available=isWeatherAvailableDate(plan.date,today),historicalPlan=isHistoricalWeatherDate(plan.date,today),loading=state.loadingTargets.has(plan.target.key),failed=state.failedTargets.has(plan.target.key);return <a className={`agenda-day-weather-report${weather?' has-forecast':' weather-unavailable'}`} key={plan.target.key} href={weatherSearchUrl(plan.target,plan.date)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>
