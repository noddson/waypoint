import { DailyWeather, formatWeatherTemperaturePair, isHistoricalWeatherDate, isWeatherAvailableDate, WeatherDayPlan, WeatherForecastState, weatherDescription, weatherSearchUrl, WeatherTemperatureUnit } from './weather'
...
export function AgendaDayWeather({plans,state,today,temperatureUnit,language}:{plans:WeatherDayPlan[];state:WeatherForecastState;today:string;temperatureUnit:WeatherTemperatureUnit;language:LanguageCode}) {
  ...
  const hasWeather=plans.some(plan=>!!weatherForPlan(plan,state)),places=formatPlaceList(plans.map(plan=>plan.target.label),language),historical=plans.every(plan=>isHistoricalWeatherDate(plan.date,today)),dates=[...new Set(plans.map(plan=>plan.date))]
  const label=dates.length===1?uiMessage('Weather for {places} on {date}',language,{places,date:shortDate(dates[0],language)}):plans.map(plan=>uiMessage('Weather for {places} on {date}',language,{places:plan.target.label,date:shortDate(plan.date,language)})).join('; ')
  return <aside className="agenda-day-weather" aria-label={label}>
   <div className="agenda-day-weather-reports">{plans.map((plan,index)=>{const weather=weatherForPlan(plan,state),available=isWeatherAvailableDate(plan.date,today),historicalPlan=isHistoricalWeatherDate(plan.date,today),loading=state.loadingTargets.has(plan.target.key),failed=state.failedTargets.has(plan.target.key),reportLabel=uiMessage('Weather for {places} on {date}',language,{places:plan.target.label,date:shortDate(plan.date,language)});return <a className={`agenda-day-weather-report${weather?' has-forecast':' weather-unavailable'}`} key={`${plan.target.key}:${plan.date}:${index}`} href={weatherSearchUrl(plan.target,plan.date)} title={reportLabel} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>
     ...
     {weather?<ForecastValues weather={weather} temperatureUnit={temperatureUnit} language={language}/>:<span className="weather-message">{uiText(!available?'Available closer to trip':failed?(historicalPlan?'Historical weather unavailable':'Forecast unavailable'):loading?'Loading…':'Open weather ↗',language)}</span>}
    </a>})}</div>
   {hasWeather&&<a className="weather-attribution" href="https://open-meteo.com/" target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>{uiText(historical?'Historical weather: Open-Meteo':'Weather: Open-Meteo',language)}</a>}
  </aside>
}
